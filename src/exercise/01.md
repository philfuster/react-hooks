...
`React.useState` tells React to re-render the component (calling the component
function again) when the state changes. This is how React knows to update the UI
when the managed state changes.

If you stored the state in a regular variable, React would not know to re-render
the component when that variable changed. So use `React.useState` if you want
React to re-render your component when the state changes updating the UI with
that change.

You can provide an initial value to `React.useState` by passing it as an
argument to the `React.useState` function. This value will be used as the
initial state for the component.

You can then have this initial value be a prop of the component and pass it in
when you use the component. This is useful when you want to have the initial
value from the user of the component.

Note: It's important to remember that the initial value may not get passed in by
the consumer, so you should always provide a default value to `React.useState`
to avoid any issues, by either defaulting the prop when destructuring or
checking for undefined when calling `React.useState` and providing the default
there like via nullish coalescing.

Another thing to keep in mind, is that for inputs that are controlled by React,
you should set the value of the input to the state value and then update the
state value when the input changes. This is how you can manage the state of the
input and have React re-render the component when the state changes.
